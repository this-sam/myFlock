...
- (void)parseToken;
- (void) sendTweetByEmail;
- (void) switchToHowReadItWithURL:(NSString *)url;
- (void) saveToRIL;
@end
...
- (void)tableView:(UITableView *)tView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (row == 0) {
		...
		if (pair.screenName) {
			...
			//Read it later patch
			[self switchToHowReadItWithURL:pair.url];
...
- (void) moreButtonAction:(id)sender {
	...
	UIActionSheet *otherButtonActionSheet = [[UIActionSheet alloc]
											 ...
											 otherButtonTitles:@"Send tweet by email", nil];
...
- (void) switchToHowReadItWithURL:(NSString *)url {
	...
	UIActionSheet *otherButtonActionSheet;	
	otherButtonActionSheet = [[UIActionSheet alloc]
							  initWithTitle:@"What would you like to do with this address?"
							  delegate:self 
							  cancelButtonTitle:@"Cancel" 
							  destructiveButtonTitle:nil
							  otherButtonTitles:@"Read it now", @"Read it later", nil];
	otherButtonActionSheet.actionSheetStyle=UIActionSheetStyleBlackOpaque;
	otherButtonActionSheet.tag = 2;
	[otherButtonActionSheet showInView:self.view];
	[otherButtonActionSheet release];
	self.navigateTo = url;
}
...
- (void) sendTweetByEmail {
	...
	NSString *subject = [NSString stringWithFormat:@"tweet from: @%@", message.screenName];
	[controller setSubject:subject];
	
	NSString *body = [NSString stringWithFormat:@"%@\n\n Sent from Tweetee http://www.corradoignoti.it/tweetee", message.text];
...
- (void) saveToRIL {
	...
	[ReadItLater save:[NSURL URLWithString:self.navigateTo] 
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if (actionSheet.tag == 1) {
		...
		switch (buttonIndex) {
			case 0: //send by email
				[self sendTweetByEmail];
				break;
			default:
				break;
		}
	...
	// how to read the link related to the tweet
	if (actionSheet.tag == 2) {
		
		switch (buttonIndex) {
			case 0: //Read it now
				[self switchToBrowserViewWithURL:navigateTo];
				break;
	
			case 1: //Read it later
				[self saveToRIL];
				break;
...
- (BOOL) favButtonImage {
	...
	//corradoignoti.it - button aren't graphical anymore
	/*
	if (message.favorited) {
		if ([[NTLNConfiguration instance] darkColorTheme]) {
...
- (UITableViewCell *)tweetCell {
	...
	int y = textHeight + 13*2;
	{
		...
		[b setFrame:CGRectMake(15, y, 70, 36)];
	...
	{
		UIButton *b = [UIButton buttonWithType:UIButtonTypeRoundedRect];
		[b setFrame:CGRectMake(15+70, y, 70, 36)];
		[b setTitle:@"favorite" forState:UIControlStateNormal];
	...
	{
		UIButton *b = [UIButton buttonWithType:UIButtonTypeRoundedRect];
		[b setFrame:CGRectMake(15+70+70, y, 70, 36)];
		[b setTitle:@"retweet" forState:UIControlStateNormal];
	...
	{
		UIButton *b = [UIButton buttonWithType:UIButtonTypeRoundedRect];
		[b setFrame:CGRectMake(15+70+70+70, y, 70, 36)];
		[b setTitle:@"more" forState:UIControlStateNormal];
		//[b setBackgroundImage:bimage[3] forState:UIControlStateNormal];
		//[b setBackgroundImage:bimage[4] forState:UIControlStateHighlighted];
		[b addTarget:self action:@selector(moreButtonAction:) forControlEvents:UIControlEventTouchUpInside];
		[cell addSubview:b];
	}
	cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (void)twitterClientSucceeded:(NTLNTwitterClient*)sender messages:(NSArray*)messages {
	...
	if ([self favButtonImage]) [favButton setTitle:@"remove fav." forState:UIControlStateNormal];
	[favAI stopAnimating];
...
- (void)twitterClientFailed:(NTLNTwitterClient*)sender {
	//[favButton setBackgroundImage:[self favButtonImage] forState:UIControlStateNormal];
	if ([self favButtonImage]) [favButton setTitle:@"not fav." forState:UIControlStateNormal];
	[favAI stopAnimating];
	[favAI removeFromSuperview];
}
